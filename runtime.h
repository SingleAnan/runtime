...
struct objc_class {
    Class _Nonnull isa  OBJC_ISA_AVAILABILITY;  // 指针 类的isa指针指向其元类（metaClass）
    ...
    struct objc_protocol_list * _Nullable protocols          OBJC2_UNAVAILABLE; // 协议楼链表
#endif
 ...
 * 3. 如果获取的变量为空,那么 ivar_getName 和 ivar_getTypeEncoding 获取的值为空,那么[NSString stringWithUTF8String:ivarName] 执行崩溃
 */
OBJC_EXPORT Ivar _Nullable
 ...
 *返回值: Ivar
 *
 * 1. 目前没有找到关于Objective-C中类变量的信息,一般认为Objective-C不支持类变量.注意,返回列表不包含父类的成员变量和属性.
 */
OBJC_EXPORT Ivar _Nullable
 ...
 * 3. 属性不存在获取 property_getName 和 property_getAttributes 会崩溃
 */
 ...
 * 返回： 变量信息Ivar列表
 * 1. 获取所有属性
 * 2. 获取的属性名和你代码一样，获取出来的属性名不自动添加下划线
 * 3. 不能获取Category添加的属性
 */
...
/**
 * 动态添加方法
 * 入参： 类Class，方法名SEL，方法实现IMP，方法返回值各个参数类型等等配置字符串
 * 返回： 添加结果，是否成功。
 * 1. 添加属性不用在 objc_registerClasspair 之前，因为添加属性其实就是添加变量的 set 和 get 方法而已
 * 2. 添加的属性和变量不能用 kvc 设置值和取值
 */
OBJC_EXPORT BOOL
...
/**
 * 方法替换
 * 入参： 要替换方法所在的类Class， 要替换的方法名SEL，方法的实现，方法的描述字符串
 * 返回： char数组
 */
OBJC_EXPORT IMP _Nullable
 ...
 * 返回： 添加结果：是否成功
 * 1. 只能给动态创建的类添加变量，也就是用 objc_allocateClassPair 创建的类
...
/**
 * 动态添加属性
 * 入参： 类Class，属性名char数组，属性的配置属性，objc_property_attribute_t，
 * 返回： 添加结果，是否成功
 */
OBJC_EXPORT BOOL
 ...
 * 入参：
 * 返回： char数组
 * 低平率函数，有兴趣的可以看官方文档https://developer.apple.com/reference/objectivec/objective_c_runtime
 */
OBJC_EXPORT void
...
struct objc_method {
    ...
    char * _Nullable method_types                            OBJC2_UNAVAILABLE; // 方法返回值和各个参数类型等的字符串描述
